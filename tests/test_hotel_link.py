"""Fix-test loop for hotel booking links — requires a live Trip.com connection.

Run directly (not via run_trip_examples):
  python tests/test_hotel_link.py
"""
...
def epoch(n: int) -> bool:
    ...
    print(
        f"epoch {n:2d}: name={name[:40]!r} url_ok={ok_url} "
        f"fake_rejected={ok_fake_rejected} name_ok={ok_name} dates={ok_has_dates}"
    )
    if ok_url:
